...
### Deep-link search params (used by newsletter email click-throughs)

`/feed` accepts two optional search params handled by `FeedPageClient`:

| Param | Effect |
|---|---|
| `?photo=<postId>` | Opens the lightbox for that PhotoPost on mount |
| `?track=<postId>` | Auto-plays that MusicPost via `MusicProvider` on mount |

Both use a `Promise.resolve().then(setState)` microtask to satisfy `react-hooks/set-state-in-effect`. These params are generated by the Go broadcaster when rendering email card click-through links — do not remove the param handling without updating the backend too.
